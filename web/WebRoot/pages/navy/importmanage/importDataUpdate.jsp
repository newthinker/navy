...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function checkinput() {
				...
				obj = document.getElementById("STR_AGREE");
				if (obj.value == "[请输入代理协议编号]") {
					obj.value = "";
				}
				
				obj = document.getElementById("STR_EXAMINENO");
				if (obj.value == "[请输入审批文件编号]") {
					obj.value = "";
				}
				...
				obj = document.getElementById("STR_CURRENCYID");
				if (!check_input(obj, false, 0, null, null, "币种")) {
					return false;
				}
				...
				obj = document.getElementById("STR_AGREE");
				cnt = document.getElementById("AGREE");
				if (obj.value == "" && cnt.innerText > 0) {
					alert("必须填写代理协议");
					obj.focus();
					obj.select();
				...
				obj = document.getElementById("STR_EXAMINENO");
				cnt = document.getElementById("EXAMINE");
				if (obj.value == "" && cnt.innerText > 0) {
					alert("必须填写二级归口审批文件");
					obj.focus();
					obj.select();
	...
	<body style="background:url(resources/images/common/tbl_bg.gif) top left repeat-x; background-color:#ffffff;" scroll="yes">
		<form action="system" method="post">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr>
					...
					<td>
						<select name="STR_CURRENCYID" id="CURRENCYID" style="width:200px"
							class="searchTbl_select" onchange="setSelectLabel('STR_CURRENCY', this);">
							<option>-请选择-</option>
							<%
								for (int i = 0; i < currency.getList("RESULT").size(); i ++) {
									DTO dtoDict = (DTO)currency.getList("RESULT").get(i);
							%>
							<option value="<%= dtoDict.getString("DICTCODE") %>"><%= dtoDict.getString("DICTNAME") %></option>
							<%
								}
					...
					<td>
						兑美元
						<input type="text" name="STR_RATE" id="STR_RATE" class="searchTbl_input" style="width:47px"
							value="<%= dto.showNumber("RATE").equals("") ? dto.showDouble("RATE") : dto.showNumber("RATE") %>" />
						&nbsp;
						兑人民币
						<input type="text" name="STR_RMBRATE" id="STR_RMBRATE" class="searchTbl_input" style="width:47px" 
							value="<%= dto.showNumber("RMBRATE").equals("") ? dto.showDouble("RMBRATE") : dto.showNumber("RMBRATE") %>" />
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<select name="STR_AGENTID" id="STR_AGENTID" style="width:200px"
							class="searchTbl_select" onchange="setSelectLabel('STR_AGENT', this);">
							<option>-请选择-</option>
							<%
								for (int i = 0; i < agent.getList("RESULT").size(); i ++) {
									DTO dtoDict = (DTO)agent.getList("RESULT").get(i);
							%>
							<option value="<%= dtoDict.getString("DICTCODE") %>"><%= dtoDict.getString("DICTNAME") %></option>
							<%
								}
					...
					<td>
						<input type="text" name="STR_AGREE" id="STR_AGREE" class="searchTbl_input" style="width:130px"
							onfocus="if (this.value == '[请输入代理协议编号]') this.value = '';" 
							onblur="if (this.value == '') this.value = '[请输入代理协议编号]';"
							value="<%= dto.getString("AGREE") == null ? "" : dto.getString("AGREE") %>" />
						<input type="button" class="btu_input" style="width:60px" value="上传文件"
							onclick="fileUpload('<%= dto.getString("AGREEID") == null ? "" : dto.getString("AGREEID") %>', 'AGREE');">
						已上传<span id="AGREE"><%= iAgreeCount %></span>个附件
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<input type="text" name="STR_EXAMINENO" id="STR_EXAMINENO" class="searchTbl_input" style="width:130px"
							onfocus="if (this.value == '[请输入审批文件编号]') this.value = '';" 
							onblur="if (this.value == '') this.value = '[请输入审批文件编号]';"
							value="<%= dto.getString("EXAMINENO") == null ? "" : dto.getString("EXAMINENO") %>" />
						<input type="button" class="btu_input" style="width:60px" value="上传文件"
							onclick="fileUpload('<%= dto.getString("EXAMINEID") == null ? "" : dto.getString("EXAMINEID") %>', 'EXAMINE');">
						已上传<span id="EXAMINE"><%= iExamineCount %></span>个附件
					</td>
				</tr>
				<tr>
					<th>
				...
				<tr>
					...
					<td>
						<select name="STR_IMPORTCLASSID" id="STR_IMPORTCLASSID" style="width:200px" class="searchTbl_select" 
							onchange="setSelectLabel('STR_IMPORTCLASS', this);selDict(this, document.getElementById('STR_DIRECTORYID'));">
							<option>-请选择-</option>
							<%
								for (int i = 0; i < impclass.getList("RESULT").size(); i ++) {
									DTO dtoDict = (DTO)impclass.getList("RESULT").get(i);
							%>
							<option value="<%= dtoDict.getString("DICTCODE") %>"><%= dtoDict.getString("DICTNAME") %></option>
							<%
								}
					...
					<td>
						<select name="STR_DIRECTORYID" id="STR_DIRECTORYID" style="width:200px"
							onchange="setSelectLabel('STR_DIRECTORY', this);">
							<option value="">-请选择-</option>
						</select>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<input name="DAT_PASSDATE" id="DAT_PASSDATE" style="cursor:hand;"
							onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',qsEnabled:true,quickSel:['2000-1-10','2000-2-20']})"
							type="text" style="width:200px;" readonly="readonly" class="Wdate"
							value="<%= dto.showDate("PASSDATE") %>" />
					</td>
				</tr>
				<tr>
					<th>
					...
					<td colspan="4">
						<div class="btu">
							<input type="button" name="save" value="保 存" class="btu_input" style="width:80px"
								onclick="if (checkinput()) submit_form('Navy', 'NavyManage', 'ImportUpdateService', '/pages/navy/importmanage/importDataUpdate.jsp');" />
		...
		<script type="text/javascript">
			...
			setSelect("STR_AGENTID", "<%= dto.getString("AGENTID") == null ? "" : dto.getString("AGENTID") %>");
			setSelect("STR_MODEID", "<%= dto.getString("MODEID") == null ? "" : dto.getString("MODEID") %>");
			setSelect("STR_IMPORTCLASSID", "<%= dto.getString("IMPORTCLASSID") == null ? "" : dto.getString("IMPORTCLASSID") %>");
			...
			setSelect("STR_CURRENCYID", "<%= dto.showString("CURRENCYID") %>");
			setClass(document.getElementById('STR_DEPTID'), document.getElementById('STR_CLASSID'));
			selDict(document.getElementById('STR_IMPORTCLASSID'), document.getElementById('STR_DIRECTORYID'));
			
			setSelect("STR_CLASSID", "<%= dto.getString("CLASSID") == null ? "" : dto.getString("CLASSID") %>");
			setSelect("STR_DIRECTORYID", "<%= dto.getString("DIRECTORYID") == null ? "" : dto.getString("DIRECTORYID") %>");
