...
{
	showx = event.screenX - event.offsetX - 1 ; // + deltaX;
	showy = event.screenY - event.offsetY + 20; // + deltaY;
	newWINwidth = 210 + 4 + 18;

       // retval = window.open("CalendarDlg.htm", "", "height=100, width=400,left="+showx+"px,top="+showy+"px, scrollbars=no, resizable=no,location=no,status=no"  );
	//window.open ('page.html', 'newwindow', 'height=100, width=400, top=0,left=0, toolbar=no, menubar=no, ') 

	retval = window.showModalDialog("resources/javascript/CalendarDlg.htm", "", "dialogWidth:188px; dialogHeight:185px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable:no;toolbar:no;location:no;help:no"  );
	if( retval != null ){
