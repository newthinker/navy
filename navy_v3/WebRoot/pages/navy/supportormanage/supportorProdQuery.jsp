	...
	<head>
		...
		<script type="text/javascript">
			function showUpdate(prodid) {
				var url = "pages/navy/supportormanage/supportorProdSaveOrUpdate.jsp?supid=<%= supid %>";
				...
				var param = "dialogLeft=" + top + "px;dialogLeft=" + left + "px;dialogWidth:600px;dialogHeight:500px;resizable:no;scroll:0;help:0;status:0";
				window.showModalDialog(url, window, param);
			}
			
			function showDetail(prodid) {
				var url = "pages/navy/supportormanage/supportorProdDetail.jsp?supid=<%= supid %>&prodid=" + prodid;
			...
			function deleteProd(prodid) {
				if (confirm('确定要删除记录吗？')) {
					var url = '/pages/navy/supportormanage/supportorProdQuery.jsp?opt=refresh&supid=<%= supid %>';
	...
	<body>
		...
		<script type="text/javascript">
			if ("<%= sOpt %>" == "null" || "<%= sOpt %>" == "refresh") {
				...
				submit_form('Navy', 'NavyManage', 'SupportorProdQueryService', '/pages/navy/supportormanage/supportorProdQuery.jsp?supid=<%= supid %>');
			}
		</script>
	</body>
</html>