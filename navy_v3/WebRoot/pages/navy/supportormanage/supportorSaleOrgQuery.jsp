	...
	<head>
		...
		<script type="text/javascript">
			function showUpdate(orgid) {
				var url = "pages/navy/supportormanage/supportorSaleOrgSaveOrUpdate.jsp?supid=<%= supid %>";
			...
			function deleteOrg(orgid) {
				if (confirm('确定要删除记录吗？')) {
					var url = '/pages/navy/supportormanage/supportorSaleOrgQuery.jsp?opt=refresh&supid=<%= supid %>';
					window.document.getElementById('str_orgid').value = orgid;
					submit_form('Navy', 'NavyManage', 'SupportorSaleOrgDeleteService', url);
				}
			}
			
			function showDetail(orgid) {
				var url = "pages/navy/supportormanage/supportorSaleOrgDetail.jsp?supid=<%= supid %>&orgid=" + orgid;
	...
	<body>
		...
		<script type="text/javascript">
			if ("<%= sOpt %>" == "null" || "<%= sOpt %>" == "refresh") {
				...
				submit_form('Navy', 'NavyManage', 'SupportorSaleOrgQueryService', '/pages/navy/supportormanage/supportorSaleOrgQuery.jsp?supid=<%= supid %>');
			}
		</script>
	</body>
</html>